...
def create_snapshots(
    ...
    user_times = users_df.set_index("id")[
        "created_at"
    ].to_dict()  # Format: {user_id: created_at}
    tweet_times = tweets_meta_df.set_index("id")["created_at"].to_dict()

    edges_df = edges_df[
        edges_df["relation"].isin(
            [
                "following",
                "followers",
                "post",
    ...
    with open(snapshots_path, "wb") as f:
        for ts in tqdm(timestamps, desc="Snapshots", total=len(timestamps)):
            ...
            active_tweets = tweets_meta_df[
                (tweets_meta_df["created_at"] <= ts)
                & (tweets_meta_df["author_id"].isin(active_users_set))
            ]["id"].tolist()

            active_edges = edges_df[(edges_df["active_from"] <= ts)][
