...
def load_users(input_dir):
    ...
    pbar = tqdm(
        total=len(users_data) // CHUNK_SIZE + 1, desc="  Processing users", unit="chunk"
    )
...
def calculate_degrees(input_dir, users_df):
    ...
    pbar = tqdm(desc="  Processing edges", unit="chunk")
...
def select_seeds(users_df, target_users, seed_ratio, hub_ratio):
    users_df["time_bin"] = pd.qcut(
        users_df["created_at"], q=20, labels=False, duplicates="drop"
    )
    ...
    for bin_id in time_bins:
        ...
        for label in ["human", "bot"]:
            ...
            hubs = label_users.nlargest(min(n_hubs, len(label_users)), "degree")[
                "user_id"
            ].values
            seeds.update(hubs)
            ...
            if len(remaining) > 0:
                random_sample = remaining.sample(
                    min(n_random, len(remaining)), random_state=42
                )["user_id"].values
...
def extract_entities(input_dir, sampled_user_ids):
    ...
    print(
        f"Extracted {len(tweets):,} tweets, {len(lists):,} lists, {len(hashtags):,} hashtags"
    )
...
def save_results(output_dir, target_users, entity_ids, all_edges, users_df):
    ...
    pd.DataFrame({"tweet_id": list(entity_ids["tweets"])}).to_csv(
        tweets_file, index=False
    )
    ...
    pd.DataFrame({"hashtag_id": list(entity_ids["hashtags"])}).to_csv(
        hashtags_file, index=False
    )
...
def plot_distributions(output_dir, original_df, sampled_df):
    ...
    axes[0].bar(
        [i - 0.2 for i in x],
        [orig_counts.get("human", 0), orig_counts.get("bot", 0)],
        width=0.4,
        label="Original",
        alpha=0.7,
    )
    axes[0].bar(
        [i + 0.2 for i in x],
        [samp_counts.get("human", 0), samp_counts.get("bot", 0)],
        width=0.4,
        label="Sampled",
        alpha=0.7,
    )
    ...
    axes[1].hist(
        original_df["degree"], bins=50, alpha=0.5, label="Original", density=True
    )
    axes[1].hist(
        sampled_df["degree"], bins=50, alpha=0.5, label="Sampled", density=True
    )
    ...
    axes[2].plot(
        bins,
        [orig_bins.get(b, 0) for b in bins],
        marker="o",
        label="Original",
        alpha=0.7,
    )
    axes[2].plot(
        bins,
        [samp_bins.get(b, 0) for b in bins],
        marker="s",
        label="Sampled",
        alpha=0.7,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "distributions.png"), dpi=150, bbox_inches="tight"
    )
    plt.close()


def sample(
    input_dir, output_dir, target_users, seed_ratio=0.1, hub_ratio=0.7, bot_ratio=0.14
):
