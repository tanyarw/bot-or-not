...
def train_epoch_sequential(
    ...
    for snapshot_dict, hetero_data in dataset.iter_snapshots():
        ...
        if cfg.SKIP_SMALL_SNAPSHOTS and hetero_data['user'].num_nodes < cfg.MIN_USERS_PER_SNAPSHOT:
            logger.warning(f"  Skipping snapshot (too few users: {hetero_data['user'].num_nodes})")
            snapshot_idx += 1
            continue
        ...
        val_loss, val_acc, val_auc = evaluator.evaluate_snapshot(model, hetero_data, 'val_mask', device)
...
if __name__ == "__main__":
    ...
    user_embeddings_path = os.path.join(cfg.DATA_DIR, "embeddings",f"user_node_embeddings_128_test.pt")
    tweet_embeddings_path = os.path.join(cfg.DATA_DIR, "embeddings", f"tweet_node_embeddings_128_test.pt")
    ...
    for epoch in range(cfg.NUM_EPOCHS):
        ...
        total_time = time.time() - time.mktime(time.strptime(current_time, "%Y-%m-%d %H:%M:%S"))
        logger.info(f"Epoch {epoch + 1} completed at {end_time} (Duration: {total_time/60:.2f} minutes)")
        ...
        if val_acc > best_val_acc:
            ...
            if cfg.SAVE_MODEL:
                ...
                logger.success(f"Best model saved (Val Acc: {val_acc:.4f} and Val AUC: {val_auc:.4f})")
        else:
            patience_counter += 1
        
        # Early stopping
        if hasattr(cfg, 'EARLY_STOPPING_PATIENCE') and patience_counter >= cfg.EARLY_STOPPING_PATIENCE:
